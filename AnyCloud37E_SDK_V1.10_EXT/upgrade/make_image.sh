...
#     resouce_upgrade=y
    ...
    echo -n "upgrade $HOME_PARTTION? [y/n]"
    read -n 2 home_upgrade
fi
...
if [ "$home_upgrade" = "y" ]; then

    value=$(wc -c <"$UPGRADE_IMAGE_DIR$HOME_PARTTION")
    echo "# File Parttion: $HOME_PARTTION $parttion_start_postion $value" >>$upgrade_bin_name
    echo "" >>$upgrade_bin_name
    parttion_start_postion=$((parttion_start_postion + value))
fi
...
# fi
if [ "$home_upgrade" = "y" ]; then
    dd if=$UPGRADE_IMAGE_DIR$HOME_PARTTION bs=512 conv=notrunc >>$upgrade_bin_name
fi
