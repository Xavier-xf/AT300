...
#define jpg_size_max (128 * 1024)
	unsigned char *jpg_data = (unsigned char *)malloc(jpg_size_max);
	if(read_logo_data(jpg_data) == -1)
	{
	...
	if(jpg_data[0] == 0xFF && jpg_data[1] == 0xD8)
	{
		...
		// uboot_logo_sjpg_decode(jpg_data, jpg_size_max, (unsigned char *)gd->fb_base);
		extern int jpeg_decode(const char *jpeg_buffer, int jpeg_size, char *frame_buffer);
		jpeg_decode(jpg_data, jpg_size_max, (unsigned char *)gd->fb_base);
	}
	else
	{
		memset((unsigned char *)gd->fb_base, 0, 1024 * 600 * 3);
		printf("error:jpg logo data NULL\n");
	}
	free(jpg_data);
