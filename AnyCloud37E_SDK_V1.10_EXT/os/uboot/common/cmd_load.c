...
static void _copy_filename(char *dst, const char *src, int size)
{
	if (*src && (*src == '"')) {
		++src;
		--size;
			      ...
			      char * const argv[])
{
	...
	if (argc == 4) {
		_copy_filename(filename, argv[3], sizeof(filename));
