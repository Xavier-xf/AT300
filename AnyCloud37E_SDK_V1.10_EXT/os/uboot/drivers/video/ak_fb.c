	...
	*/
	if (fb_dev->panel_info->rgb_if == 0x1){
        ...
        if (fb_dev->reset_pin_active < 0) {
			printf("could not get mipi pannel device reset pin polarity\n");
			ret = 1;
			goto lcd_para_exit;
        }
		...
		fb_dev->reset_pin_active = fdtdec_get_valoffset_int(blob, node, "reset-pins", 2, -1);
        if (fb_dev->reset_pin_active < 0) {
			printf("could not get rgb pannel device reset pin polarity\n");
			ret = 1;
			goto lcd_para_exit;
        }
        fb_dev->reset_pin_active = !fb_dev->reset_pin_active;
...
#ifdef CONFIG_37_E_CODE
		fb_dev->pwr_gpio = fdtdec_get_valoffset_int(blob, node, "pwr-gpio", 1, -1);
		if (fb_dev->pwr_gpio < 0) {
			printf("could not get lcd device power pin\n");
			ret = 1;
			goto lcd_para_exit;
		}
        fb_dev->pwr_gpio_active = fdtdec_get_valoffset_int(blob, node, "pwr-gpio", 2, -1);
        if (fb_dev->pwr_gpio_active < 0) {
			printf("could not get lcd device power pin polarity\n");
			ret = 1;
			goto lcd_para_exit;
        }
        fb_dev->pwr_gpio_active = !fb_dev->pwr_gpio_active;
#endif
...
int ak_lcd_parameter_parse_dt3(struct ak_fb *fb_dev)
{
	...
	panel_info->thd = fdtdec_get_int(blob, node_sta, "panel-thd", panel_info->width);
	panel_info->tvd = fdtdec_get_int(blob, node_sta, "panel-tvd", panel_info->height);
#if 0
	printf("%s,line:%d, fdt_addr:0x%x\n", __func__, __LINE__, blob);
...
static int ak_lcd_parameter_parse_dt4(struct ak_fb *fb_dev)
{
	struct lcdc_panel_info *panel_info = fb_dev->panel_info;
	unsigned int node_sta;
	const void *blob;
	...
	panel_info->height = fdtdec_get_int(blob, node_sta, "panel-height", 0);
	panel_info->pclk_div = fdtdec_get_int(blob, node_sta, "panel-pclk-div", 0);
	panel_info->data_width = fdtdec_get_int(blob, node_sta, "panel-data-width", 0);
	panel_info->color_mode = fdtdec_get_int(blob, node_sta, "panel-color-mode", 0);
	panel_info->a0_pol = fdtdec_get_int(blob, node_sta, "panel-rs-polarity", 0);
	panel_info->wr_cycle = fdtdec_get_int(blob, node_sta, "panel-wr-cycle", 0);
	panel_info->wr_low = fdtdec_get_int(blob, node_sta, "panel-wr-low", 0);
	panel_info->panel_direction = fdtdec_get_int(blob, node_sta, "panel-direction", 0);

#if 0
	printf("panel_info->width=%d\n", panel_info->width);
	printf("panel_info->height=%d\n", panel_info->height);
	printf("panel_info->pclk_div=%d\n", panel_info->pclk_div);
	printf("panel_info->data_width=%d\n", panel_info->data_width);
	printf("panel_info->color_mode=%d\n", panel_info->color_mode);
	printf("panel_info->a0_pol=%d\n", panel_info->a0_pol);
	printf("panel_info->wr_cycle=%d\n", panel_info->wr_cycle);
	printf("panel_info->wr_low=%d\n", panel_info->wr_low);
	printf("panel_info->panel_direction=%d\n", panel_info->panel_direction);
#endif
...
int ak_lcd_parameter_parse_dt1(const void *blob, struct ak_fb *fb_dev)
{
    ...
    fb->dev->pwr_gpio_active = LCD_PWR_PIN_ACTIVE;
#else
    fb->dev->pwr_gpio_active = 0;
#endif
#endif
...
static int ak_lcd_parameter_parse_dt4(struct ak_fb *fb_dev)
{
	...
	panel_info->width = PANEL_4_WIDTH; // fdtdec_get_int(blob, node_sta, "panel-width", 0);
	panel_info->height = PANEL_4_HEIGHT; // fdtdec_get_int(blob, node_sta, "panel-height", 0);
	panel_info->pclk_div = PANEL_4_PCLK_DIV; // fdtdec_get_int(blob, node_sta, "panel-pclk-div", 0);
	panel_info->data_width = PANEL_4_DATA_WIDTH; // fdtdec_get_int(blob, node_sta, "panel-data-width", 0);
	panel_info->color_mode = PANEL_4_COLOR_MODE; // fdtdec_get_int(blob, node_sta, "panel-color-mode", 0);
	panel_info->a0_pol = PANEL_4_A0_POL; // fdtdec_get_int(blob, node_sta, "panel-rs-polarity", 0);
	panel_info->wr_cycle = PANEL_4_WR_CYCLE; // fdtdec_get_int(blob, node_sta, "panel-wr-cycle", 0);
	panel_info->wr_low = PANEL_4_WR_LOW; // fdtdec_get_int(blob, node_sta, "panel-wr-low", 0);
	panel_info->panel_direction = PANEL_4_DIRECTION; // fdtdec_get_int(blob, node_sta, "panel-direction", 0);
...
static void rgb_pannel_sharepin_cfg(struct ak_fb *fb_dev)
{
	...
	if(fb_dev->panel_info->bus_width == LCD_RGB_BUS_WIDTH_8BIT){
        ...
        */
		...
		regval &= ~((0x3<<4)|(0x3<<6)|(0x3<<8)|(0x3<<10)|(0x3<<12)|(0x3<<14)|(0x3<<16)|(0x3<<18)|(0x3<<20)|(0x3<<22)|(0x3<<24)|(0x3<<26));
        ...
        */
		regval = __raw_readl(SHARE_PIN_CFG5_REG);
		regval &= ~((0x7<<0)|(0x7<<3)|(0x7<<6)|(0x7<<9));
		regval |= ((0x1<<0) |(0x1<<3)|(0x1<<6)|(0x1<<9));
		__raw_writel(regval, SHARE_PIN_CFG5_REG);
        ...
        */
		...
		regval &= ~((0x3<<0)|(0x3<<2)|(0x3<<4)|(0x3<<6)|(0x3<<8)|(0x3<<10));
        ...
        */
		...
		regval &= ~((0x7<<18)|(0x7<21));
		regval |= ((0x4<<18)|(0x4<<21));
...
static void mpu_panel_init(struct ak_fb *fb_dev)
{
    ...
    if (fb_dev->reset_pin >= 0) {
		...
		gpio_direction_output(36, fb_dev->reset_pin_active);

		mdelay(10);/* Delay 10ms*/
		gpio_set_pin_level(36, !fb_dev->reset_pin_active);
        mdelay(10);/* Delay 10ms*/
		gpio_set_pin_level(36, fb_dev->reset_pin_active);
        mdelay(120);/* Delay 120ms*/
...
static void mipi_panel_init(struct ak_fb *fb_dev)
{
	...
	if (fb_dev->reset_pin > 0) {
        ...
        // gpio_set_pin_level(fb_dev->reset_pin, !fb_dev->reset_pin_active);
        // mdelay(120);/* Delay 120ms*/
		gpio_direction_output(41, 0);//vdd
		gpio_direction_output(34, 0);//reset
		gpio_direction_output(36, 0);//avdd
		mdelay(10);/* Delay 10ms*/
		gpio_set_pin_level(41, 1); // vdd拉高10ms
		mdelay(10);				   /* Delay 10ms*/
		gpio_set_pin_level(34, 1); // reset拉高5ms
		mdelay(5);				   /* Delay 5ms*/
		gpio_set_pin_level(34, 0); // reset拉低
		mdelay(20);				   /* Delay 20ms*/
		gpio_set_pin_level(36, 1); // avdd拉高20ms
		mdelay(20);				   /* Delay 20ms*/
#else
        gpio_direction_output(fb_dev->reset_pin, 1);
        gpio_set_pin_level(fb_dev->reset_pin, 1);
...
static void lcdc_info_cfg(struct ak_fb *fb_dev)
{
	if(fb_dev->panel_info->rgb_if == DISP_IF_RGB){
		...
		*/
		...
		mpu_panel_sharepin_cfg(fb_dev);
        mpu_panel_init(fb_dev);
	}
...
static void lcdc_rgb_display(struct ak_fb *fb_dev)
{
	...
	{	
		if (fb_dev->panel_info->rgb_if == DISP_IF_MPU) {
            /*send writting MPU GRAM command: 0x2c*/
			mpu_write_reg(LCD_MPU_CMD, MPU_WRITE_DATA_CMD,fb_dev);
			__raw_writel(0x1<<3, AK_LCD_GO);
            while(1){
                regval = __raw_readl(AK_LCD_INT_STATUS);
                if((regval & (1<<18)) == (1<<18))
                    debug("fifo alarm!\n");
...
/*
 * mpu_scan_dir -
 * set scanning method of mpu lcd
 *
 * @fb_dev:         pointor to fb device
 * @dir:            scanning method
 */
static void mpu_scan_dir(struct ak_fb *fb_dev, u32 dir)
{
    struct lcdc_panel_info *panel_info = fb_dev->panel_info;
    u16 regval=0;
    u8 dirreg=0;
    ...
    width = (u16)panel_info->width;
    height = (u16)panel_info->height;
    ...
    dirreg=MPU_DIRECTION_REG;

    regval|=0x08;

    mpu_write_reg(AK_LCD_MPU_CMD,dirreg,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,regval,fb_dev);

    mpu_write_reg(AK_LCD_MPU_CMD,panel_info->setxcmd,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,0,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,0,fb_dev);

    mpu_write_reg(AK_LCD_MPU_DATA,(width-1)>>8,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,(width-1)&0XFF,fb_dev);

    mpu_write_reg(AK_LCD_MPU_CMD,panel_info->setycmd,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,0,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,0,fb_dev);

    mpu_write_reg(AK_LCD_MPU_DATA,(height-1)>>8,fb_dev);
    mpu_write_reg(AK_LCD_MPU_DATA,(height-1)&0XFF,fb_dev);
}

/*
 * mpu_display_dir -
 * set scanning method of mpu lcd, and get scanning mothod command
 *
 * @fb_dev:         pointor to fb device
 * @dir:            scanning method
 */
static void mpu_display_dir(struct ak_fb *fb_dev,u32 dir)
{
    struct lcdc_panel_info *panel_info = fb_dev->panel_info;
    u32 default_scan_dir = 0;

    /* portrait*/
    if(dir==0)
    {
        panel_info->dir=0;
        panel_info->wramcmd=0X2C;
        panel_info->setxcmd=0X2A;
        panel_info->setycmd=0X2B;
        //DFT_SCAN_DIR=U2D_R2L;
        default_scan_dir=L2R_U2D;
    }else/* landscape */
	{
		panel_info->dir=1;
        panel_info->wramcmd=0X2C;
        panel_info->setxcmd=0X2A;
        panel_info->setycmd=0X2B;
        default_scan_dir=U2D_L2R;
    }
    mpu_scan_dir(fb_dev,default_scan_dir);
}
...
static void lcd_panel_on(struct ak_fb *fb_dev)
{
	...
	*/
    ...
    if (fb_dev->panel_info->rgb_if == DISP_IF_MPU) {
        mpu_display_dir(fb_dev,fb_dev->panel_info->panel_direction);
    }
	...
	*/
	mdelay(95);
	gpio_direction_output(CONFIG_LCD_BACKLIGHT_GPIO, fb_dev->backlight_active);
...
void lcd_ctrl_init(void *lcdbase)
{
	...
	*/
	...
	fb_dev->panel_info->paddr = (dma_addr_t)(lcdbase);
	...
	*/
    ...
    fb_dev->backlight_active = CONFIG_LCD_BACKLIGHT_GPIO_ACTIVE;
#else
    fb_dev->backlight_active = 1;
#endif
#endif
    ...
    gpio_direction_output(CONFIG_LCD_BACKLIGHT_GPIO, !fb_dev->backlight_active);
    mdelay(20);
    gpio_direction_output(fb_dev->pwr_gpio, fb_dev->pwr_gpio_active);
	printf("pwr_gpio:%d pwr_gpio_active:%d\n", fb_dev->pwr_gpio, fb_dev->pwr_gpio_active);
#else
    gpio_direction_output(CONFIG_LCD_BACKLIGHT_GPIO, 0);
    mdelay(20);
    gpio_direction_output(fb_dev->pwr_gpio, 0);
