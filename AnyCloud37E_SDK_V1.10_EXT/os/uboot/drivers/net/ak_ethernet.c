...
static s32 gmac_phy_init(u8 dev_num, gmac_device *gmacdev)
{
	...
	/* reset */
	// gmac_phy_gpio_reset(dev_num, gmacdev); // wxj:硬复位后需要一定时间重新协商
	...
	if(data_id1 == 0x22) {
        ...
        /*** 切换到mii寄存器下�? ***/
        gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x100);
        gmac_read_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1f, &data_ctl);
        data_ctl |= 0x02;
        gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x100);
        gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1f, data_ctl);
        ...
        gmac_read_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, PHY_AN_ADV_REG, &data_ctl);
        data_ctl |= ((0x01 << 8) | (0x01 << 5));
        gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, PHY_AN_ADV_REG, data_ctl);
...
static s32 gmac_check_phy_init(gmac_device *gmacdev)
{
	...
	*/
    ...
    while (loop_count--)
    {
        ...
        if((data & Mii_AutoNegCmplt) != 0 || (data & Mii_Link) != 0){
    ...
    gmacdev->Speed = SPEED10;
    gmacdev->DuplexMode = FULLDUPLEX;
    printf("Link is with 10M Speed, FULLDUPLEX mode \n");
#endif
    return -ESYNOPGMACNOERR;
}
...
static int smc_init(struct eth_device *dev, bd_t *bd)
{
    ...
    if(gmac_check_phy_init(gmacdev) != 0)
    {
        return -1;
    }
...
static void gmac_rmii_interface_50mclk(u8 dev_num)
{
...
#else // 10M
        REG32(AK_PA_SYSCTRL + 0x14) &=	~(0x1 << 23); // cdh:first  mac_speed_cfg=0(10m)
#endif
...
#else // 10M
        REG32(AK_PA_SYSCTRL + 0x14) &=	~(0x1 << 21); // cdh:first  mac_speed_cfg=0(10m)
#endif
	...
	*/
	...
	REG32(AK_PA_SYSCTRL + 0x14) |=  (0x1 << 23); // cdh:first  mac_speed_cfg=1(100m)
#else // 10M
    REG32(AK_PA_SYSCTRL + 0x14) &=  ~(0x1 << 23); // cdh:first  mac_speed_cfg=1(10m)
#endif
