...
static unsigned char ecc_bits = 2;
...
static 	struct spinand_flash_status_reg status_reg_list[] = {
		...
		{	//XT26G01C
			.jedec_id = 0x1100110b, .ext_id = 0,
			.b_wip = 0,	.b_wel = 1,	.b_efail= 2,	.b_pfail= 3,
			.b_qe = 8,
		},
		{	//XT26G02C
			.jedec_id = 0x1200120b, .ext_id = 0,
			.b_wip = 0,	.b_wel = 1,	.b_efail= 2,	.b_pfail= 3,
			.b_qe = 8,
		},
		/*normal status reg define*/
...
static struct spinand_flash_info ak_spinand_param_supportlist [] = {
	...
	{ "FM25S01BI3", 0xd4a1d4a1, 0, 1, 2048, 64, 128*1024, 1024, 64, 0, 12, 4, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},//OK
	...
	{ "GD5F1GM7UE", 0x91c891c8, 0, 1, 2048, 64, 128*1024, 1024, 64, 0, 12, 4, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
	{ "GD5F2GM7UE", 0x92c892c8, 0, 1, 2048, 64, 128*1024, 2048, 64, 0, 12, 4, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
	...
	{ "XT26G01C", 0x1100110b, 0, 1, 2048, 64, 128*1024, 1024, 64, 0, 16, 0, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
	{ "MX35LF2GE4AD", 0x000326c2, 0, 1, 2048, 64, 128*1024, 2048, 64, 0, 12, 4, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
	{ "XT26G02C", 0x1200120b, 0, 1, 2048, 64, 128*1024, 2048, 64, 0, 16, 0, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
    
    { "HYF4GQ4UAACBE", 0xd4c9d4c9, 0, 1, 4096, 64, 4096*64, 2048, 256, 0, 4, 28, 8, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
};
...
static int check_ECC_status(struct spi_slave *spi)
{
	...
	if(4 == ecc_bits)
	{
		// printf("check ecc status 4 bits\r\n");
		ecc = (status >> 4) & 0x0f;
		if(8 < ecc)
		{
			printf("r ecc error sr:0x%x\r\n", status);
			return -1;
		}
...
int spinand_flash_write_inter(struct spinand_flash *flash, u32 offset, size_t len, const void *data,  const void *oob, size_t oob_len)
{
	...
	oob_buf = malloc(sizeof(u8) * (flash->oob_size));
	if(!oob_buf)
	{
		printf("error:oob_buf malloc fail.\r\n");
		return -1;
	}
	buf = malloc(sizeof(u8) * (flash->oob_size + flash->page_size));
	if(!buf)
	{
		printf("error:oob_buf or buf malloc fail.\r\n");
		free(oob_buf);
		return -1;
	}
    
	memset(oob_buf, 0xFF, flash->oob_size);
	memset(buf, 0xFF, flash->oob_size + flash->page_size);
...
int spinand_flash_read_oob(struct spinand_flash *flash, u32 offset, void *oob, size_t oob_len)
{
	...
	buf = malloc(sizeof(u8) * flash->oob_size);
	if(!buf)
	{
		printf("error:malloc fail\r\n");
		return -1;
	}
	memset(buf, 0xFF, flash->oob_size);
...
static struct spinand_flash *spinand_flash_validate_params(struct spi_slave *spi, u8 *idcode)
{
	...
	if(0x1100110b == flash->flashid || 0x1200120b == flash->flashid)
	{
		ecc_bits = 4;
	}
