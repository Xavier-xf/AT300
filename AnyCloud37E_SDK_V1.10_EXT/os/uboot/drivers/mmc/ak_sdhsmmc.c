...
static int ak_mmc_sd_sharepin_cfg(int mmcdev_index)
{
	...
	if (mmcdev_index == 0) { // cdh:mmc/sd
		...
		// wxj ��ǿsd0���ŵ��������� GPIO:68 69 71 73 76 77
		value = REG32(PAD_DRV_CFG4_REG);
		value &= ~((0x3 << 8) | (0x3 << 10) | (0x3 << 14) | (0x3 << 18) | (0x3 << 24) | (0x3 << 26));
		value |= ((0x3 << 8) | (0x3 << 10) | (0x3 << 14) | (0x3 << 18) | (0x3 << 24) | (0x3 << 26));
		REG32(PAD_DRV_CFG4_REG) = value;
	}else if (mmcdev_index == 1) { // cdh:sdio
		...
		*/
		...
		// wxj ��ǿsd1���ŵ��������� GPIO:16 17 18 19 20 21
		value = REG32(PAD_DRV_CFG1_REG);
		value &= ~((0x3 << 0) | (0x3 << 2) | (0x3 << 4) | (0x3 << 6) | (0x3 << 8) | (0x3 << 10));
		value |= ((0x3 << 0) | (0x3 << 2) | (0x3 << 4) | (0x3 << 6) | (0x3 << 8) | (0x3 << 10));
...
bool sd_init_info(int idex, struct mmc_cd_info *mmc_cd)
{
	...
	if(fdtdec_get_bool((const void *)s_fdt_dtb, node, "power-pins")){
	...
	if(fdtdec_get_bool((const void *)s_fdt_dtb, node, "cd-clk")){
#else
