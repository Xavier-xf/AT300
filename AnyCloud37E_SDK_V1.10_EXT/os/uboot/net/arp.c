...
# define ARP_TIMEOUT		500UL//5000UL
#else
...
# define ARP_TIMEOUT_COUNT	3//5	/* # of timeouts before giving up  */
#else
	...
	IPaddr_t targetIP)
{
	...
	pkt = NetArpTxPacket;  // cdh:(len=14 + 28 )
...
void ArpRequest(void)
{
	if ((NetArpWaitPacketIP & NetOurSubnetMask) !=
		...
		if (NetOurGatewayIP == 0) {
			printf("## Warning1: gatewayip needed but not set\n");
			NetArpWaitReplyIP = NetArpWaitPacketIP;
		} else {
			NetArpWaitReplyIP = NetOurGatewayIP;
			printf("## Warning2: gatewayip needed but not set\n");
		}
	} else {
		NetArpWaitReplyIP = NetArpWaitPacketIP;
		printf("## Warning3: gatewayip needed but not set\n");
	}
...
void ArpReceive(struct ethernet_hdr *et, struct ip_udp_hdr *ip, int len)
{
	...
	case ARPOP_REPLY:		/* cdh: ping arp reply ok*/
		...
		if (reply_ip_addr == NetArpWaitReplyIP) {
			...
			//printf("cdh:net_get_arp_handler1:0x%x\n", net_get_arp_handler());
			...
			memcpy(((struct ethernet_hdr *)NetTxPacket)->et_dest,
				&arp->ar_sha, ARP_HLEN);
			//printf("cdh:arp_receive:NetTxPacket:0x%x, NetArpWaitTxPacketSize:%d\n", NetTxPacket, NetArpWaitTxPacketSize);
			NetSendPacket(NetTxPacket, NetArpWaitTxPacketSize);
			//printf("cdh:net_get_arp_handler2:0x%x\n", net_get_arp_handler());
			//printf("cdh:net_get_arp_handler3:0x%x\n", net_get_arp_handler());
			//printf("cdh:net_get_arp_handler4:0x%x\n", net_get_arp_handler());
