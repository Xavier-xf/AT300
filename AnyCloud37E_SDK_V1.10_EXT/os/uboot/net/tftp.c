	    ...
	    unsigned len)
{
	...
	case TFTP_ACK:
		...
		if (TftpWriting) {
			if (TftpFinalBlock) {
				printf("cdh:tftp_complete TftpFinalBlock\n");
				tftp_complete();
			} else {
				/*
...
#endif
		if (len < TftpBlkSize) {
			//printf("cdh:if tftp_complete len:0x%x < TftpBlkSize:0x%x\n", len, TftpBlkSize);
			tftp_complete();
		}
		//else{
		//	printf("cdh:else tftp_complete len:0x%x < TftpBlkSize:0x%x\n", len, TftpBlkSize);
		//}
