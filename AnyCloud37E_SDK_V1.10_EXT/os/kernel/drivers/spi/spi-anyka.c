...
int ak_spi_set_clk(struct spi_device *spi, unsigned long clk)
{
    int ret;
    struct ak_spi *hw = to_hw(spi);

    /* reduce freq before read id for compatible with gm25qxx*/
    ret = clk_set_rate(hw->bus_clk, clk);
    if (ret) {
        pr_err("%s: clk_set_rate fail, return %d\n", __func__, ret);
        return ret;
    }
    pr_info("<%s> SPI%d bus frequency is %lu HZ\n", __func__, spi->master->bus_num, clk_get_rate(hw->bus_clk));

    return 0;
}
EXPORT_SYMBOL(ak_spi_set_clk);
				    ...
				    struct spi_transfer *t)
{
	...
	if(spi->master->bus_num == 0){
		spi->max_speed_hz = spi->max_speed_hz > hw->bus_freq ?
							hw->bus_freq : spi->max_speed_hz;
	}
...
static int  ak_spi_probe(struct platform_device *pdev)
{
	...
	if(master->bus_num == 0){
		...
		hw->bus_freq = spi_bus_freq;
	} 

#endif
    hw->clk = clk_get(&pdev->dev, NULL);
	pr_info("enter %s..., hw=%p, hw->clk=%p\n", __func__,hw, hw->clk);
	if (IS_ERR(hw->clk)) {
...
static struct dev_pm_ops ak_spi_pmops = {
	SET_SYSTEM_SLEEP_PM_OPS(ak_spi_suspend,ak_spi_resume)
};
