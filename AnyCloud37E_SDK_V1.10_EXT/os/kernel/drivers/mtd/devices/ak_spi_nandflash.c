...
static int check_ecc_status(struct ak_spinand *flash)
{
	...
	do {
		...
		if(0x0b110011 == flash->info.jedec_id || 0x0b120012 == flash->info.jedec_id)
		{
			// printk("check ecc status 4 bits\r\n");
			if(((sr >> 4) & 0x0f) > 8){
		...
		size_t *retlen, u_char *buf)
{
	...
	*retlen = 0;
	...
	column = (from & ( (1<<flash->page_shift) - 1));
...
static int spinand_do_read_page(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	r_buftmp = vmalloc(sizeof(uint8_t) * (flash->info.oob_size + flash->info.page_size));
		...
		size_t *retlen, const u_char *buf)
{
	...
	*retlen = 0;
	...
	column = (to & ( (1<<flash->page_shift) - 1));
...
static int spinand_do_write_page(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	u32 buftmp_len = 0;	
	uint8_t *w_buftmp_oob = NULL;
	int i;
	uint8_t *w_buftmp = NULL;
	uint8_t *oobbuf = ops->oobbuf;
	uint8_t *datbuf = ops->datbuf;
	struct ak_spinand *flash = mtd_to_spiflash(mtd);
	...
	w_buftmp_oob = vmalloc(sizeof(uint8_t) * flash->info.oob_size);
	w_buftmp = vmalloc(sizeof(uint8_t) * (flash->info.oob_size + flash->info.page_size));
...
int ak_spinand_probe(struct spi_device *spi)
{
	...
	flash->mtd.writebufsize = flash->info.page_size; /* ubifs will use this variable */
