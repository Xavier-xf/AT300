...
extern int ak_spi_set_clk(struct spi_device *spi, unsigned long clk);
...
static int winbond_fix_erase_disturb(struct ak_spiflash *flash)
{
	int ret;
...
static int ak_spiflash_get_devid(struct mtd_info *mtd)
{
    ...
    max_speed_hz = flash->spi->max_speed_hz;

    /* reduce frequence before read flash id to competible wite GM25XXXX */
    ret = ak_spi_set_clk(flash->spi, 50000000);
    if (ret != 0)
        pr_err("<%s> set spi clk to 50M fail, may lead to read flash ID fail\n", __func__);
	...
	*/
    ...
    /* resume max_speed_hz*/
    ret = ak_spi_set_clk(flash->spi, max_speed_hz);
    if (ret != 0)
        pr_err("<%s> resume max speed hz fail\n", __func__);
...
static struct flash_info * jedec_probe(struct spi_device *spi)
{
    ...
    /* reduce frequence before read flash id to competible wite GM25XXXX */
    tmp = ak_spi_set_clk(spi, 50000000);
    if (tmp != 0)
        pr_err("<%s> set spi clk to 50M fail, may lead to read flash ID fail\n", __func__);
    ...
    if ((jedec != 0x1c4018) && (jedec != 0x1c4017)) {
        tmp = ak_spi_set_clk(spi, max_speed_hz);
        if (tmp != 0)
            pr_err("<%s> resume max speed hz fail\n", __func__);
    }
...
static int spiflash_get_partition_data(char *partition_name, char *data, unsigned long *date_len)
{
    ...
    if ((*date_len > 0) && (partition_size > *date_len)) { // avoid write too much
        //pr_err("partition_size %ld date_len %ld\n", partition_size, *date_len);
        partition_size = *date_len;
    }
...
static int  ak_spiflash_probe(struct spi_device *spi)
{
	...
	if (info->flags & SFLAG_FREQ_SEL_50M)
    ...
    **/
    if(info->jedec_id == 0xef4017 || info->jedec_id == 0xef4018){
        winbond_fix_erase_disturb(flash);
    }
