/*
 * linux/arch/arm/mach-anycloud/include/mach/ak_l2_dual_inter.h
 *
...
#ifndef __ASM_ARCH_L2_DUAL_INNER_H__
#define __ASM_ARCH_L2_DUAL_INNER_H__
 ...
 * 0 - 7: L2 common buffer, UART/SPI0/DAC/ADC/MMC/SD/SDIO/512Bytes
 * 8 - 15: Dedicate L2 buffer for UART
 * 16 - 23: L2 common buffer, SPI1/SPI2/USB
 * 24: USB
 */
#define L2_0_COMMON_BUFFER_NUM			8
#define L2_0_UART_BUFFER_NUM			8
#define L2_1_COMMON_BUFFER_NUM			8
#define L2_1_USB_DEDICATE_BUFFER_NUM	1

#define L2_COMMON_BUFFER_NUM			(L2_0_COMMON_BUFFER_NUM + L2_1_COMMON_BUFFER_NUM)
#define L2_UART_BUFFER_NUM				(L2_0_UART_BUFFER_NUM)

#define L2_UART_BUFFER_INDEX			(L2_0_COMMON_BUFFER_NUM)
#define L2_USB_HOST_BUFFER_INDEX		(L2_0_COMMON_BUFFER_NUM + L2_0_UART_BUFFER_NUM)

#define L2_0_BUF_NUM					(L2_0_COMMON_BUFFER_NUM + L2_0_UART_BUFFER_NUM)
#define L2_1_BUF_INDEX					(L2_0_COMMON_BUFFER_NUM + L2_0_UART_BUFFER_NUM)
#define L2_2_BUF_INDEX					(L2_0_COMMON_BUFFER_NUM + L2_0_UART_BUFFER_NUM + L2_1_COMMON_BUFFER_NUM)

#define L2_BUF_MAX						(L2_0_COMMON_BUFFER_NUM + L2_0_UART_BUFFER_NUM + L2_1_COMMON_BUFFER_NUM)

#define L2_COMMON_BUFFER_LEN			512
#define L2_UART_BUFFER_LEN				128

#define L2_COMMON_BUFFER_OFFSET			0
#define L2_UART_BUFFER_OFFSET			(L2_COMMON_BUFFER_LEN * L2_0_COMMON_BUFFER_NUM)

#define L2_BUF_0						0
#define L2_BUF_1						1

#define L2_BUF_COMM_NUM					8
/*
 * L2 controller working clock define
 */
#define L2_0_CLOCK_EN				(0x1 << 9)
#define L2_1_CLOCK_EN				(0x1 << 11)
...
typedef enum {
	...
	ADDR_SPI0_RX,			/* Rx buffer of SPI0 Controller */
	ADDR_SPI0_TX,			/* Tx buffer of SPI0 Controller */
	ADDR_DAC,				/* DAC control module */	
	ADDR_L2_0_P1_MAX,
	ADDR_PDM = ADDR_L2_0_P1_MAX,
	ADDR_ADC,
	ADDR_L2_0_P2_MAX,
	ADDR_USB_EP1 = ADDR_L2_0_P2_MAX,/* USB 2.0 HS Controller: Endpoint 1 */
	...
	ADDR_L2_1_P1_MAX,
	ADDR_SPI2_RX = ADDR_L2_1_P1_MAX,/* Rx buffer of SPI2 Controller */
	ADDR_SPI2_TX,			/* Tx buffer of SPI2 Controller */
	ADDR_USB_EP4,			/* USB 2.0 HS Controller: Endpoint 3 */
	ADDR_USB_EP5,			/* USB 2.0 HS Controller: Endpoint 4 */
	ADDR_USB_EP6,			/* USB 2.0 HS Controller: Endpoint 5 */
	ADDR_USB_EP0,			/* USB 2.0 HS Controller: Endpoint 6 */
	ADDR_MAX,				/* Max of the l2_device_t*/
} l2_device_t;


#define BUF_NULL					0xFF	/* Invalid L2 buffer ID */
#define L2_UART_BUF_START_ID		L2_0_COMMON_BUFFER_NUM	/* UART used buffer ID from 8 */
...
void l2_cpu(unsigned long ram_addr, u8 id,unsigned long buf_offset, unsigned int bytes, l2_dma_transfer_direction_t direction);
...
/**
 * l2_get_dma_trans_len - L2 buffer get dma transfer len
 *  @id:	L2 buffer ID
 */
u32 l2_get_dma_trans_len(u8 id);

/**
 * l2_wait_clear_dma - wait and clear L2 buffer dma
 *  @id:	L2 buffer ID
 */
void l2_wait_clear_dma(u8 id);

static inline void l2_enable_clock(int enable)
{
	if (enable)
		REG32(L2_CLOCK_REG) &= ~(L2_0_CLOCK_EN | L2_1_CLOCK_EN);
	else
		REG32(L2_CLOCK_REG) |= (L2_0_CLOCK_EN | L2_1_CLOCK_EN);
	return;
}

#endif	/* __ASM_ARCH_L2_DUAL_INNER_H__ */
