...
void l2_cpu(unsigned long ram_addr, u8 id,
...
static void l2_select_combuf(l2_device_t device, u8 id)
{
	...
	if (((u8)device) < ADDR_L2_0_P1_MAX) {
		...
		switch ((u8)device) {
			...
			case ADDR_SPI1_TX:
			case ADDR_SPI1_RX:
				bits_offset = ((u8)device - ADDR_L2_0_P2_MAX + 4) * 3;
				break;
		}
		buf_assignmemt = res + rL2_BUFASSIGN1;
	} else {
		switch ((u8)device) {
			...
			case ADDR_USB_EP6:
			case ADDR_USB_EP0:
				bits_offset = ((u8)device - ADDR_L2_1_P1_MAX + 3) * 3;
				break;
		}
		buf_assignmemt = res + rL2_BUFASSIGN2;
	}
...
EXPORT_SYMBOL(l2_wait_clear_dma);
...
u32 l2_get_dma_trans_len(u8 id)
{
	...
	u8 idx = l2_get_buf_index(id);
    u32 reg;
    u32 cnt_cur_h, cnt_cur, cnt_cfg;
    u32 len;

	l2_assert_buf_id(id);

    reg = __raw_readl(res + vL2DMA_CONBUF0 + idx * 4);
	cnt_cur_h = (reg >> 24) & 0xFF;
	cnt_cur = (reg >> 8) & 0xFF;
	cnt_cfg = (reg >> 0) & 0xFF;

	if(cnt_cur_h > 0){
	    len = cnt_cur_h * cnt_cfg * 64;
        len += cnt_cur * 64;
	}
	else{
	    len = cnt_cur * 64;
...
EXPORT_SYMBOL(l2_get_dma_trans_len);
