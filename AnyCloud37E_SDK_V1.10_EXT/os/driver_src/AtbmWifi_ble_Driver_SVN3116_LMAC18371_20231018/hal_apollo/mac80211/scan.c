...
static void __ieee80211_scan_cancel(struct ieee80211_local *local);
#endif
			  ...
			  bool beacon,bool notransmit,u8 *transmitbssid)
{
	...
	*/
...
#endif
#ifdef ATBM_USE_FASTLINK
		if(local->is_associated && !memcmp(mgmt->bssid, local->associated_mac, ETH_ALEN))
		{
		memcpy(local->write_all,&len,sizeof(len));
		memcpy(local->write_all+sizeof(len),mgmt,len);
	...
	if(hw_priv->scan.req &&  hw_priv->scan.req->n_ssids > 0){
			...
			bool beacon)
{
	...
	atbm_for_each_element_id(elem, ATBM_WLAN_EID_MULTIPLE_BSSID, ie, ielen) {
		...
		atbm_for_each_element(sub, elem->data + 1, elem->datalen - 1) {
			...
			if(new_ie_len > ielen){
				atbm_printk_err("new_ie_len (%d),ielen(%d)\n",new_ie_len,ielen);
				new_ie_len = ielen;
			}
				       ...
				       bool was_hw_scan)
{
	...
	*/
	...
	if (was_hw_scan && !aborted && ieee80211_prep_hw_scan(local->scan_sdata)) {
