...
//	u8 ucDbgPrintOpenFlag = 1;
	...
	*/
	...
	if(len)
	{
		...
		*parase precom
		*/
		ATBM_WEXT_PROCESS_PARAMS(pos,len,precom,atbm_accsii_to_int,false,exit,ret);
		switch(precom){
		case 1://FCC
		case 2:
			if(bw != 0)
...
next:
		...
		value_packet_time = value_packet_time / 32;//iwpriv æ˜¯usï¼?lmacæ˜?2us
...
void  __PHY_RF_TX_Cal_Force_Set(u8 PowerIndex, Power_LUT_bit *pforcetable, Power_LUT_bit *pMemoryGainTable)
{
	...
	while(pMemoryGainTable->addr != PHY_RFIP_INVALID)
	{
		...
		if(idx == 1)
		{
			regval = 0x5A;//pow(10.0,(regval/4/20.0))*pow(2.0,6);//dB to linear
		}
