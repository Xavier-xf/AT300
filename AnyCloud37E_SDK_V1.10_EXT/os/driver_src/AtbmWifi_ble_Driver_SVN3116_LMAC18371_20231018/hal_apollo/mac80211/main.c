...
static void atbm_save_bss_work(struct work_struct *work)
{
    ...
    loff_t pos;
	size_t len;
	struct ieee80211_local *local = container_of(work, struct ieee80211_local, save_bss_work);
	
    fp =filp_open(ATBM_SAVE_BSS,O_RDWR | O_CREAT,0777);
    if (IS_ERR(fp)){
        atbm_printk_err("create file error/n");
    }else{
        fs = get_fs();
        set_fs(KERNEL_DS);
        pos=0;
    	len = *((size_t *)local->write_all);
        //kernel_write(fp,local->write_all, len+sizeof(len), &pos);
		kernel_write(fp,local->write_all,512,fp->f_pos);
        filp_close(fp,NULL);
        set_fs(fs);
	}
}
...
#ifdef ATBM_USE_FASTLINK
	ATBM_INIT_WORK(&local->save_bss_work,atbm_save_bss_work);
#endif
...
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,12,0) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3,16,0)))
...
#ifdef ATBM_USE_FASTLINK
	atbm_cancel_work_sync(&local->save_bss_work);
#endif
