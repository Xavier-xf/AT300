...
*/
	...
	}*/
	{
		...
		atbm_printk_wext("**ETF_PHY_Start_Tx***\n");
		ETF_PHY_TxRxParamInit(&EtfConfig);
		EtfConfig.TxConfig.ChannelNum = channel;
		EtfConfig.TxConfig.FreqMHz    = ieee80211_channel_to_frequency(channel, NL80211_BAND_2GHZ);
		EtfConfig.TxConfig.Rate = rate;
...
static int atbm_dev_set_ant_control(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	if(trigger_probe == 1){
		...
		scan_ch_list[0] = scan_ch_list[1] = scan_ch_list[2] = get_work_channel(sdata);
		internal_scan.channels = scan_ch_list;
		internal_scan.n_channels = 3;
		if(atbm_internal_cmd_scan_triger(sdata,&internal_scan) == false){
