...
struct ble_gap_adv_params {
    /** connect mode. Can be one of following constants:
...
struct ble_gap_ext_adv_params {
	...
	unsigned int auxiliary_packet:1;
    uint32_t itvl_min;
    uint32_t itvl_max;
    uint8_t channel_map;
...
/**int8_t dbm (-10 ~ 10 dbm)**/
int ble_gap_set_default_tx_power(int8_t dbm);

int ble_gap_adv_active_instance(uint8_t instance);
void ble_gap_adv_finished(uint8_t instance, int reason, uint16_t conn_handle, uint8_t num_events);
