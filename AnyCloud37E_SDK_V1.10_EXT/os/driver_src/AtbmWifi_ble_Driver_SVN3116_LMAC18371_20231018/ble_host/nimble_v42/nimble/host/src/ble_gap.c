                 ...
                 const struct ble_gap_conn_params *params)
{
    ...
    BLE_HS_LOG(INFO, " scan_itvl=%d scan_window=%d itvl_min=%d itvl_max=%d "
                     "latency=%d super_tmo=%d min_ce_len=%d "
...
ble_gap_ext_auxiliary_pkt_set(uint8_t instance,const struct ble_gap_ext_adv_params *params)
{
	...
	uint8_t buf[2];
	buf[0] = instance;
	buf[1] = params->auxiliary_packet;
   	rc = ble_hs_hci_cmd_tx_empty_ack(BLE_HCI_OP(BLE_HCI_OGF_VENDOR,
                                 BLE_HCI_OCF_LE_AUXILIARY_PACKET_SET),
                                 buf, sizeof(buf));
	return rc;
}
SRAM_CODE static int
...
ble_gap_ext_adv_params_validate(const struct ble_gap_ext_adv_params *params)
{
    ...
    if (params->legacy_pdu) {
        ...
        if (params->anonymous || params->include_tx_power || params->auxiliary_packet ) {
            return BLE_HS_EINVAL;
        }
    }
    ...
    if (!params->legacy_pdu) {
		...
		if(params->auxiliary_packet == 0 && (params->connectable || params->scannable)){
			return BLE_HS_EINVAL;
		}
                          ...
                          ble_gap_event_fn *cb, void *cb_arg)
{
	...
	if(!params->legacy_pdu){
		rc = ble_gap_ext_auxiliary_pkt_set(instance, params);
	    if (rc) {
			printf("ble_gap_ext_auxiliary_pkt_set\n");
	        ble_hs_unlock();
	        return rc;
	    }
    }
...
SRAM_CODE static struct ble_gap_update_entry *
