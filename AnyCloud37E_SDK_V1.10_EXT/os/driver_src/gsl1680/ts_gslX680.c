...
static int gslX680_shutdown_high(void)
{
	...
	// pr_info("ts_gsl reset OK, Set the reset pin direction to input mode!\n");
...
#if 0//wxj iic读写测试
	char buf = 0;
	int ret = gsl_ts_read(client, 0xf0, &buf, 1);
	if (ret < 0)
	{
		pr_info(" gsl_ts_read : 11 I2C transfer error!\n");
	}
	pr_info("1111gsl_ts_read:[%d]\n", buf);


	buf = 0x12;
	gsl_ts_write(client, 0xf0, &buf, 1);
	if (ret < 0)
	{
		pr_info("gsl_ts_write  :  I2C transfer error!\n");
	}

	ret = gsl_ts_read(client, 0xf0, &buf, 1);
	if (ret < 0)
	{
		pr_info(" gsl_ts_read : 22 I2C transfer error!\n");
	}
	pr_info("2222gsl_ts_read:[%d]\n", buf);
