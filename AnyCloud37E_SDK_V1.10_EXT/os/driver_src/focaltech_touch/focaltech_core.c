...
{
    //FTS_INFO("start reading touch data"); // 增加入口日志
    int i = 0;
    ...
    //FTS_INFO("touch event type: %d", touch_etype); // 打印事件类型
    switch (touch_etype) {
    case TOUCH_DEFAULT:
    ...
    case TOUCH_EVENT_NUM:
     //FTS_INFO("detected %d touch points", event_num); // 打印触摸点数
...
{
    struct fts_ts_data *ts_data = fts_data;
    //FTS_INFO("irq triggered! irq=%d", irq); // 增加中断触发日志
