...
static int vqe_res_handle_id = -1; // resample handle
static int vqe_eq_handle_id = -1; // eq handle
static int vqe_aslc_handle_id = -1; // aslc handle
...
{
    /* open vqe */
    if (ak_vqe_open(AK_VQE_RESAMPLE, &vqe_res_handle_id)) {
        return AK_FAILED;
    }
    ...
    if (ak_vqe_set_res_attr(vqe_res_handle_id, &pcm_attr_in, &pcm_attr_out)) {
        return AK_FAILED;
    }
    return AK_SUCCESS;
}
...
{
    if (-1 != vqe_res_handle_id) {
        /* vqe close */
        ak_vqe_close(vqe_res_handle_id);
        vqe_res_handle_id = -1;
    }
}

/**
 * init_vqe_eq - open vqe, init eq parameter
 * notes: 
 */
static int init_vqe_eq(void)
{
    /* open vqe */
    if (ak_vqe_open(AK_VQE_EQ, &vqe_eq_handle_id)) {
        return AK_FAILED;
    }
    ...
    if (ak_vqe_set_eq_attr(vqe_eq_handle_id, &pcm_attr_in, &ai_eq_attr)) {
        return AK_FAILED;
    }
    return AK_SUCCESS;
}
...
/**
 * destroy_vqe_eq - close vqe
 * notes: 
 */
static void destroy_vqe_eq(void)
{
    if (-1 != vqe_eq_handle_id) {
        /* vqe close */
        ak_vqe_close(vqe_eq_handle_id);
        vqe_eq_handle_id = -1;
    }
}

/**
 * init_vqe_aslc - open vqe, init aslc parameter
 * notes: 
 */
static int init_vqe_aslc(void)
{
    /* open vqe */
    if (ak_vqe_open(AK_VQE_ASLC, &vqe_aslc_handle_id)) {
        return AK_FAILED;
    }
    struct ak_audio_data_attr pcm_attr_in;
    ...
    if (ak_vqe_set_aslc_attr(vqe_aslc_handle_id, &pcm_attr_in, volume)) {
        return AK_FAILED;
    }
    return AK_SUCCESS;
}
...
/**
 * destroy_vqe_aslc - close vqe
 * notes: 
 */
static void destroy_vqe_aslc(void)
{
    if (-1 != vqe_aslc_handle_id) {
        /* vqe close */
        ak_vqe_close(vqe_aslc_handle_id);
        vqe_aslc_handle_id = -1;
    }
}
...
static void* ai_capture_loop(void *arg)
{
    ...
    while (1) {
        ...
        ret = ak_vqe_process(vqe_eq_handle_id, frame.data, frame.len, &eq_data, &eq_len);
        if (ret)
            break;
        ret = ak_vqe_process(vqe_res_handle_id, eq_data, eq_len, &resample_data, &resample_len);
        if (ret)
            break;
        ret = ak_vqe_process(vqe_aslc_handle_id, resample_data, resample_len, &aslc_data, &aslc_len);
        if (ret)
            break;
        ...
        if (NULL != fp) {
            if(fwrite(aslc_data, aslc_len, 1, fp) < 0) {
 ...
 */
#ifdef AK_RTOS
static int ai_vqe_sample(int argc, char *argv[])
#else
...
#endif
{
    ...
    if (init_vqe_eq()) {
        ak_print_error_ex(MODULE_ID_APP, "init vqe eq failed\n");
        goto vqe_end;
    }
    if (init_vqe_resample()) {
        ak_print_error_ex(MODULE_ID_APP, "init vqe resample failed\n");
        goto vqe_end;
    }
    if (init_vqe_aslc()) {
        ak_print_error_ex(MODULE_ID_APP, "init vqe aslc failed\n");
        goto vqe_end;
    }
...
#ifdef AK_RTOS
SHELL_CMD_EXPORT_ALIAS(ai_vqe_sample, ak_ai_vqe_sample, ai vqe sample);
#endif
