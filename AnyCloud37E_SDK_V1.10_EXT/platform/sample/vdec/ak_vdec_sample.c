...
static char gc_thread_send = AK_TRUE;
/* decoder resolution */
...
static void signal_exit( int i_sig )
{
    ak_print_normal(MODULE_ID_APP, "recv SIGNAL %d\n", i_sig );
    gc_run = AK_FALSE ;
    return ;
}
...
void *vdec_send_decode_th(void *arg)
{
    ...
    fclose(fp);
    gc_thread_send = AK_FALSE;
    ak_print_normal_ex(MODULE_ID_APP, "vdec_send_decode_th exit\n");
    return NULL;
}
...
void *vdec_solo_frame(void *arg)
{
    ...
    while ( ( gc_run == AK_TRUE ) && ( entry = readdir(dir ) )  )
    {
        /* if get the file */
...
exit:
    ...
    refresh_flag &= ~(1 << *id);
    ak_thread_mutex_unlock(&refresh_flag_lock);
    ak_print_normal_ex(MODULE_ID_APP, "vdec_get_frame_th exit\n");
    return NULL;
}
...
__STATIC__ int main(int argc, char **argv)
{
    ...
    struct ak_vo_layer_in video_layer = {0};
    ...
    video_layer.format   = GP_FORMAT_RGB888;          /* layer format , only support the rgb except the yuv format */
