...
static char ac_option_hint[  ][ LEN_HINT ] = {
    ...
    "[PATH] 录像保存路径,默认值:'./dvr'" ,
    "[SEC]  录像时长,默认值:'10'" ,
    "[NUM]  录像录制文件个数,0为不限制录像数量,默认值:'1'" ,
    "[NUM]  采样率 可选值: 8000,11025,12000,16000,22050,24000,32000,44100,48000,默认值:'8000'" ,
    "[NUM]  采集音量设置0-8,默认值:'6'" ,
    "[FILE] sensor的配置文件,默认值:'/etc/config/isp.conf'" ,
    "[NUM]  分辨率宽,默认值:'1920'" ,
    "[NUM]  分辨率高,默认值:'1080'" ,
    "[NUM]  帧率,默认值:'25'" ,
    "[NUM]  I帧出现频率,默认值:'50'" ,
    "[NUM]  音频编码格式 AVI格式可选值:     - 2,MP3 4,AAC 17,(g711a)PCM_ALAW 18,(g711u)PCM_ULAW 6,PCM\n"
    "                                                  3gp/mp4格式可选值: - 3,AMR 4,AAC 17,(g711a)PCM_ALAW 18,(g711u)PCM_ULAW,默认值:'4'",
    "[NUM]  venc编码类型 可选值: 0,H264 1,JPEG 2,H265,默认值:'0'" ,
    "[NUM]  合成文件类型 可选值: 0,MUX_TYPE_AVI 1,MUX_TYPE_MP4. JPEG编码格式仅仅支持AVI,默认值:'1'" ,
    "[STR]  目录模板,默认:\%Y\%M\%D           可选值:\%Y-年,\%M-月,,\%D-日,\%h-时,\%m-分,\%s-秒,\%S-录制长度ms,默认值:'%Y%M%D'" ,
    "[STR]  文件模板,默认:\%Y\%M\%D.\%h\%m\%s.\%S 可选值:\%Y-年,\%M-月,,\%D-日,\%h-时,\%m-分,\%s-秒,\%S-录制长度ms,默认值:'%Y%M%D.%h%m%s.%S'" ,
    "[PATH] 录像合成的暂存目录,默认值:'./dvr/tmp'" ,
    "[SIZE] 待合成的视频缓存长度,设置为0则不限制,默认值:'2097152'" ,
    "[SIZE] 待合成的音频缓存长度,设置为0则不限制,默认值:'131072'" ,
    "" ,
};

static struct option option_long[ ] = {
    { "help"           , no_argument       , NULL , 'h' } ,                     //"       打印帮助信息" ,
    { "path_save"      , required_argument , NULL , 'a' } ,                     //"[PATH] 录像保存路径,默认值:'./dvr'" ,
    { "record_sec"     , required_argument , NULL , 'b' } ,                     //"[SEC]  录像时长,默认值:'10'" ,
    { "record_num"     , required_argument , NULL , 'c' } ,                     //"[NUM]  录像录制文件个数,0为不限制录像数量,默认值:'1'" ,
    { "ai_sample_rate" , required_argument , NULL , 'd' } ,                     //"[NUM]  采样率 可选值: 8000,11025,12000,16000,22050,24000,32000,44100,48000,默认值:'8000'" ,
    { "ai_volume"      , required_argument , NULL , 'e' } ,                     //"[NUM]  采集音量设置0-8,默认值:'6'" ,
    { "vi_isp_cfg"     , required_argument , NULL , 'f' } ,                     //"[FILE] sensor的配置文件,默认值:'/etc/config/isp.conf'" ,
    { "vi_width"       , required_argument , NULL , 'g' } ,                     //"[NUM]  分辨率宽,默认值:'1920'" ,
    { "vi_height"      , required_argument , NULL , 'i' } ,                     //"[NUM]  分辨率高,默认值:'1080'" ,
    { "vi_fps"         , required_argument , NULL , 'j' } ,                     //"[NUM]  帧率,默认值:'25'" ,
    { "venc_goplen"    , required_argument , NULL , 'k' } ,                     //"[NUM]  I帧出现频率,默认值:'50'" ,
    { "aenc_type"      , required_argument , NULL , 'l' } ,                     //"[NUM]  音频编码格式 AVI格式可选值:     - 2,MP3 4,AAC 17,(g711a)PCM_ALAW 18,(g711u)PCM_ULAW 6,PCM\n"
                                                                                //"                                                  3gp/mp4格式可选值: - 3,AMR 4,AAC 17,(g711a)PCM_ALAW 18,(g711u)PCM_ULAW,默认值:'4'",
    { "venc_type"      , required_argument , NULL , 'm' } ,                     //"[NUM]  venc编码类型 可选值: 0,H264 1,JPEG 2,H265,默认值:'0'" ,
    { "mux_type"       , required_argument , NULL , 'n' } ,                     //"[NUM]  合成文件类型 可选值: 0,MUX_TYPE_AVI 1,MUX_TYPE_MP4. JPEG编码格式仅仅支持AVI,默认值:'1'" ,
    { "path_template"  , required_argument , NULL , 'o' } ,                     //"[STR]  目录模板,默认:\%Y\%M\%D           可选值:\%Y-年,\%M-月,,\%D-日,\%h-时,\%m-分,\%s-秒,\%S-录制长度ms,默认值:'%Y%M%D'" ,
    { "file_template"  , required_argument , NULL , 'p' } ,                     //"[STR]  文件模板,默认:\%Y\%M\%D.\%h\%m\%s.\%S 可选值:\%Y-年,\%M-月,,\%D-日,\%h-时,\%m-分,\%s-秒,\%S-录制长度ms,默认值:'%Y%M%D.%h%m%s.%S'" ,
    { "path_tmp"       , required_argument , NULL , 'q' } ,                     //"[PATH] 录像合成的暂存目录,默认值:'./dvr/tmp'" ,
    { "buff_video"     , required_argument , NULL , 'r' } ,                     //"[SIZE] 待合成的视频缓存长度,设置为0则不限制,默认值:'2097152'" ,
    { "buff_audio"     , required_argument , NULL , 's' } ,                     //"[SIZE] 待合成的音频缓存长度,设置为0则不限制,默认值:'131072'" ,
    { 0                ,                 0 ,    0 ,  0  } ,                     //"" ,
};
    ...
    while( p_dvr_conf->i_run == AK_TRUE ) {
        ...
        if ( ( ret = ak_aenc_get_stream( p_dvr_conf->i_aenc_handle , &p_aenc_stream_entry->stream, 0 ) ) == AK_SUCCESS ) {
            ...
            if ( ( p_dvr_conf->i_buff_audio <= 0 ) ||
                ...
                PRINTD( CM_NORMAL , CB_BLACK, CF_RED , "*** AUDIO BUFF FULL *** p_dvr_conf->i_buff_audio= %d p_dvr_conf->i_list_audio= %d\n", p_dvr_conf->i_buff_audio, p_dvr_conf->i_list_audio );
            }
            MUTEX_UNLOCK_AENC;
        }
        else {
...
static void* thread_vi_venc( void *pv_conf )                                    //录像venc采集线程
{
    ...
    while( p_dvr_conf->i_run == AK_TRUE ) {
        ...
        if ( ( ak_vi_get_frame( VIDEO_CHN0, &vi_frame_get) == AK_SUCCESS ) &&
             ( ( c_encode_frame = ak_venc_encode_frame( p_dvr_conf->i_venc_handle,
                                                        vi_frame_get.vi_frame.data, vi_frame_get.vi_frame.len, vi_frame_get.mdinfo,
                                                        &video_stream_get ) ) == AK_SUCCESS ) &&
             ( ( p_venc_stream_entry = ( struct venc_stream_entry *)calloc( 1, sizeof( struct venc_stream_entry ) ) ) != NULL ) &&
             ( ( ( p_venc_stream_entry->stream.data = ( unsigned char * )malloc( video_stream_get.len ) ) ) != NULL ) ) {         //获取一帧yuv视频及编码一帧数据
            ...
            if  ( ( p_venc_stream_entry->stream.frame_type == FRAME_TYPE_I ) ||
                  ( ( p_dvr_conf->venc_param.enc_out_type == MJPEG_ENC_TYPE ) &&
                    ( ( p_venc_stream_entry->stream.seq_no - 1 ) % p_dvr_conf->venc_param.goplen == 0 ) ) ) {      //如果是视频流判断是否当前帧是I帧，如果是JPEG编码则判断是否达到统计帧数
                ...
                if( i_ts_prev != 0 ) {                                      //测试代码,判断sensor的采集周期
                    ...
                    ak_print_normal_ex( MODULE_ID_APP,
                                        "i_times_venc= %d i_ts_prev= %llu vi_frame_mark.vi_frame.ts= %llu diff= %llu i_us_split= %llu buff_V= %.2f%% buff_A= %.2f%% fps= %.4f\n" ,
            ...
            if ( ( p_dvr_conf->i_buff_video <= 0 ) ||
                ...
                PRINTD( CM_NORMAL , CB_BLACK, CF_RED , "*** VIDEO BUFF FULL *** p_dvr_conf->i_buff_video= %d p_dvr_conf->i_list_video= %d\n", p_dvr_conf->i_buff_video, p_dvr_conf->i_list_video );
            }
            MUTEX_UNLOCK_VENC;
        }
        else {
...
static int set_mux_status( struct dvr_conf *p_dvr_conf )                        //获取当前媒体库状态,获取成功:AK_SUCCESS 失败:AK_FAILED
{
...
static int add_aenc_less_than_ts( struct dvr_conf *p_dvr_conf, unsigned long long i_ts )         //在该录像结束前,合成下一个视频首个I帧时间戳之前的全部音频帧
{
    ...
    for( ;; ) {
        ...
        list_for_each_entry_safe( p_aenc_stream_now, p_aenc_stream_next, &p_dvr_conf->list_head_aenc, list ) {
            if ( p_aenc_stream_now->stream.ts < i_ts ) {
                ...
                if ( ( p_dvr_conf->c_mux_init == AK_TRUE ) &&
                     ( p_dvr_conf->c_mux_enable == AK_TRUE ) &&
                     ( ak_mux_add_audio( p_dvr_conf->i_mux_handle, &p_aenc_stream_now->stream ) != AK_SUCCESS ) ) {
                    set_mux_status( p_dvr_conf );
                    ak_print_error_ex( MODULE_ID_APP,
                                       "****** ADD AUDIO FRAME ERROR ****** p_dvr_conf->i_mux_handle= %d p_aenc_stream_now->stream => data= %p len= %d ts= %llu seq_no= %lu\n",
...
static void* thread_mux( void *pv_conf )                                        //录像合成线程
{
    ...
    while( p_dvr_conf->i_run == AK_TRUE ) {
        ...
        list_for_each_entry_safe( p_venc_stream_now, p_venc_stream_next, &p_dvr_conf->list_head_venc, list ) {//遍历venc链表,获取一帧编码数据
            ...
            if ( ( c_gop == AK_TRUE ) &&
                 ( ( p_dvr_conf->c_mux_init == AK_FALSE ) ||
                   ( p_dvr_conf->c_mux_enable == AK_FALSE ) ||
                   ( ( p_venc_stream_now->stream.ts - i_ts_start + MS_CUT_ADD ) >= p_dvr_conf->i_sec * S2MS ) ) ) { //判断是否已经开始录像,判断录像时长是否需要截断并开始新的录像,录像期间是否出错

                if ( ( p_dvr_conf->c_mux_init == AK_TRUE ) &&
                     ( ( p_dvr_conf->c_mux_enable == AK_FALSE ) ||
                       ( ( p_venc_stream_now->stream.ts - i_ts_start + MS_CUT_ADD ) >= p_dvr_conf->i_sec * S2MS ) ) ) { //已经开始录像或者录制期间出错则保存录像
                    p_dvr_conf->i_record_ms = p_venc_stream_now->stream.ts - i_ts_start ;       //获取录像时长(毫秒)
                    c_list_exist = add_aenc_less_than_ts( p_dvr_conf, p_venc_stream_now->stream.ts );      //在结束录像前先把该时间戳前面的音频数据添加完成
            ...
            if ( ( p_dvr_conf->c_mux_init == AK_TRUE ) &&
                ...
                set_mux_status( p_dvr_conf );
                ak_print_error_ex( MODULE_ID_APP,
                                   "****** ADD VIDEO FRAME ERROR ****** p_dvr_conf->i_mux_handle= %d p_venc_stream_now->stream => data= %p len= %d ts= %llu seq_no= %lu \n",
                                   p_dvr_conf->i_mux_handle, p_venc_stream_now->stream.data, p_venc_stream_now->stream.len, p_venc_stream_now->stream.ts, p_venc_stream_now->stream.seq_no );
            }
            c_list_exist = AK_TRUE;
...
static int ak_dvr_fix( struct dvr_conf *p_dvr_conf )
{
    ...
    for( i = 0; i < i_num_scandir; i ++ ) {
        ...
        if ( strcmp( pp_dirent_list[ i ]->d_name + strlen( pp_dirent_list[ i ]->d_name ) - strlen( FILE_EXT_AVI ) , FILE_EXT_AVI ) == 0 ) {
            ...
            snprintf( ac_file_name[ MUX_NODE_IDX ], LEN_FILE, "%s%s" , ac_file_name[ MUX_NODE_RECORD ], ac_mux_ext[ MUX_NODE_IDX ] );
            if ( ( ( pFILE_mux[ MUX_NODE_RECORD ] = fopen( ac_file_name[ MUX_NODE_RECORD ], "rb+" ) ) != NULL ) &&
                 ...
                 ( ak_mux_fix_file( pFILE_mux[ MUX_NODE_RECORD ], pFILE_mux[ MUX_NODE_IDX ], NULL, NULL, MUX_TYPE_AVI, &mux_fileinfo_param ) == AK_SUCCESS ) &&
    ...
    dir = opendir(p_dvr_conf->ac_path_tmp);//dir open
    if (dir == NULL)
    {
        ak_print_error_ex(MODULE_ID_APP, "can't open file dir[%s]\n", p_dvr_conf->ac_path_tmp);
        ret = AK_FAILED;
        return ret;
    }
    while ((entry = readdir(dir))) {
        ...
        if ( strcmp( entry->d_name + strlen( entry->d_name ) - strlen( FILE_EXT_AVI ) , FILE_EXT_AVI ) == 0 ) {
            PRINTD( CM_BOLD, CB_BLACK, CF_BLUE , "****** TRY REPAIR FILE: '%s' ******\n", entry->d_name );
            snprintf( ac_file_name[ MUX_NODE_RECORD ], LEN_FILE, "%s/%s", p_dvr_conf->ac_path_tmp, entry->d_name );
            snprintf( ac_file_name[ MUX_NODE_IDX ], LEN_FILE, "%s%s" , ac_file_name[ MUX_NODE_RECORD ], ac_mux_ext[ MUX_NODE_IDX ] );
            if ( ( ( pFILE_mux[ MUX_NODE_RECORD ] = fopen( ac_file_name[ MUX_NODE_RECORD ], "rb+" ) ) != NULL ) &&
                 ( ( pFILE_mux[ MUX_NODE_IDX ] = fopen( ac_file_name[ MUX_NODE_IDX ], "rb" ) ) != NULL ) &&
                 ( ak_mux_fix_file( pFILE_mux[ MUX_NODE_RECORD ], pFILE_mux[ MUX_NODE_IDX ], NULL, NULL, MUX_TYPE_AVI, &mux_fileinfo_param ) == AK_SUCCESS ) &&
                 ( mux_fileinfo_param.total_ms > 0 ) ) {
                save_fix_file( p_dvr_conf,  entry->d_name, mux_fileinfo_param.total_ms, MUX_TYPE_AVI );
            }
        }
        else if ( strcmp( entry->d_name + strlen( entry->d_name ) - strlen( FILE_EXT_MP4 ) , FILE_EXT_MP4 ) == 0 ) {
            PRINTD( CM_BOLD, CB_BLACK, CF_BLUE , "****** TRY REPAIR FILE: '%s' ******\n", entry->d_name );
            snprintf( ac_file_name[ MUX_NODE_RECORD ], LEN_FILE, "%s/%s", p_dvr_conf->ac_path_tmp, entry->d_name );
            snprintf( ac_file_name[ MUX_NODE_IDX ], LEN_FILE, "%s%s" , ac_file_name[ MUX_NODE_RECORD ], ac_mux_ext[ MUX_NODE_IDX ] );
            pFILE_mux[ MUX_NODE_IDX ] = fopen( ac_file_name[ MUX_NODE_IDX ], "rb" );
            ...
            if ( ( ( pFILE_mux[ MUX_NODE_RECORD ] = fopen(  ac_file_name[ MUX_NODE_RECORD ], "rb+") ) != NULL ) &&
                 ...
                 ( ak_mux_fix_file( pFILE_mux[ MUX_NODE_RECORD ], NULL, pFILE_mux[ MUX_NODE_MP4_MOOV ], pFILE_mux[ MUX_NODE_MP4_STBL ], MUX_TYPE_MP4, &mux_fileinfo_param ) == AK_SUCCESS ) &&
                 ( mux_fileinfo_param.total_ms > 0 ) ) {
                save_fix_file( p_dvr_conf,  entry->d_name, mux_fileinfo_param.total_ms, MUX_TYPE_MP4 );
            }
        }
        for( m = 0; m < MUX_NODE_NUM ; m ++ ) {
            if ( pFILE_mux[ m ] != NULL ) {
...
static int ak_dvr_exit( struct dvr_conf *p_dvr_conf )
{
    ...
    if( p_dvr_conf->i_mux_handle != HANDLE_NOT_INIT ) {
        list_for_each_entry_safe( p_frame_now, p_frame_next, &p_dvr_conf->list_head_ai, list ) {    //退出时候将ai链表内容释放
            if( p_frame_now != NULL ) {
                MUTEX_LOCK_AI;
        ...
        list_for_each_entry_safe( p_aenc_stream_now, p_aenc_stream_next, &p_dvr_conf->list_head_aenc, list ) { //退出时候将aenc链表内容释放
            if( p_aenc_stream_now != NULL ) {
                MUTEX_LOCK_AENC;
        ...
        list_for_each_entry_safe( p_venc_stream_now, p_venc_stream_next, &p_dvr_conf->list_head_venc, list ) { //退出时释放venc链表内容
            if( p_venc_stream_now != NULL ) {
                MUTEX_LOCK_VENC;
