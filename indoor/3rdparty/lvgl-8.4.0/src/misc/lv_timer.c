...
/**
 * @name: 
 * @msg: 遍历定时器链表，清除未上锁定时器
 * @return {*}
 */
void lv_timer_recycle(void)
{
    void * node;
    for(node = _lv_ll_get_head(&LV_GC_ROOT(_lv_timer_ll)); node != NULL; )
    {
        lv_timer_t *tmp =(lv_timer_t *)node;
        node = _lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll), node);
        if (tmp->lock == false)
        {
            lv_timer_del(tmp);
        }
...
/**
 * @name: 
 * @msg: 自定义layout定时器创建，使用goto_layout函数时调用lv_timer_clean清除
 * @param {lv_timer_cb_t} timer_xcb
 * @param {uint32_t} period
 * @param {lv_timer_prio_t} prio
 * @param {void} *user_data
 * @return {*}
 */
lv_timer_t *lv_timer_recycle_create(lv_timer_cb_t timer_xcb, uint32_t period, void * user_data)
{
    lv_timer_t *timer_t = lv_timer_create(timer_xcb, period, user_data);
    timer_t->lock = false;//取消定时器上锁，调用lv_timer_clean会清除
    return timer_t;
}
