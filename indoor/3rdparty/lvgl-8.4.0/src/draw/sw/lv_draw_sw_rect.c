    ...
    if(src_type == LV_IMG_SRC_SYMBOL) {
        ...
        if(res == LV_RES_OK) {
            ...
            if(dsc->bg_img_tiled == false) {
                ...
                if (dsc->bg_img_align == LV_ALIGN_TOP_LEFT) {
                    area.x1 = coords->x1;
                    area.y1 = coords->y1;
                }
                else if (dsc->bg_img_align == LV_ALIGN_TOP_MID) {
                    area.x1 = coords->x1 + lv_area_get_width(coords) / 2 - header.w / 2;
                    area.y1 = coords->y1;
                }
                else if (dsc->bg_img_align == LV_ALIGN_TOP_RIGHT) {
                    area.x1 = coords->x1 + lv_area_get_width(coords) - header.w;
                    area.y1 = coords->y1;
                }
                else if (dsc->bg_img_align == LV_ALIGN_BOTTOM_LEFT) {
                    area.x1 = coords->x1;
                    area.y1 = coords->y1 + lv_area_get_height(coords) - header.h;
                }
                else if (dsc->bg_img_align == LV_ALIGN_BOTTOM_MID) {
                    area.x1 = coords->x1 + lv_area_get_width(coords) / 2 - header.w / 2;
                    area.y1 = coords->y1 + lv_area_get_height(coords) - header.h;
                }
                else if (dsc->bg_img_align == LV_ALIGN_BOTTOM_RIGHT) {
                    area.x1 = coords->x1 + lv_area_get_width(coords) - header.w;
                    area.y1 = coords->y1 + lv_area_get_height(coords) - header.h;
                }
                else if (dsc->bg_img_align == LV_ALIGN_LEFT_MID) {
                    area.x1 = coords->x1 ;
                    area.y1 = coords->y1 + lv_area_get_height(coords) - header.h;
                }
                else if (dsc->bg_img_align == LV_ALIGN_RIGHT_MID) {
                    area.x1 = coords->x1 + lv_area_get_width(coords) - header.w;
                    area.y1 = coords->y1 + lv_area_get_height(coords) / 2 - header.h / 2;
                }
                else {
