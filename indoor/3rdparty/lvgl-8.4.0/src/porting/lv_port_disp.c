 ...
 *  STATIC VARIABLES
 **********************/
static void *gui_disp_dev_ctx = NULL;
/**********************
 *      MACROS
 **********************/
...
void lv_port_disp_video(bool en)
{
    db_hal_gui_display_ioctl(gui_disp_dev_ctx, GUI_DISPLAY_VIDEO_ENABLE_CMD, &en, NULL);
}
...
{
    /*You code here*/
    gui_disp_dev_ctx = db_hal_gui_display_open(NULL);
}
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    if (disp_flush_enabled)
    {
        ...
        db_gui_framebuffer_frame_t frame;
        frame.pos.x = area->x1;
        frame.pos.y = area->y1;
        frame.pos.w = lv_area_get_width(area);
        frame.pos.h = lv_area_get_height(area);

        frame.data = (unsigned char *)color_p;

        db_hal_gui_display_write(gui_disp_dev_ctx, &frame, sizeof(frame));

        if (lv_disp_flush_is_last(disp_drv))
        {
            db_hal_gui_display_ioctl(gui_disp_dev_ctx, GUI_DISPLAY_FRAME_REFRESH_CMD, NULL, NULL);
        }
    }
