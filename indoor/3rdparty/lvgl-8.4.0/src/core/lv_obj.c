...
void lv_obj_add_state(lv_obj_t * obj, lv_state_t state)
{
    ...
    if(obj->state != new_state) {
        ...
        lv_obj_t* next = (lv_obj_t*)obj->bind_obj;
        while ((next) && (next != obj)) {
            lv_obj_set_state(next, new_state);
            next = (lv_obj_t*)next->bind_obj;
        }
    }
}
...
lv_obj_t* lv_obj_get_by_id(lv_obj_t* parent, int id)
{
    uint32_t child_cnt = 0;
    if (parent->spec_attr) {
...
#if 0
        lv_obj_t* obj = lv_obj_get_by_id(child, id);
        if (obj) {
            return obj;
        }
#endif
...
int lv_obj_set_id(lv_obj_t* obj, int id)
{
#if 0
    ...
    lv_obj_t* child = lv_obj_get_by_id(parent, id);
    if (child) {
        LV_LOG_ERROR("duplicate id:%d\n", id);
        return -1;
    }
#else
    // if(obj == NULL) 
...
int lv_obj_get_id(lv_obj_t* obj)
{
    return obj->id;
}

int lv_obj_bind(lv_obj_t* obj, lv_obj_t* bind) 
{
    lv_obj_t* next = (lv_obj_t*)bind->bind_obj;
    while ((next) && (next->bind_obj != obj)) {
        next = next->bind_obj;
    }
    if (next) {
        LV_LOG_ERROR("lv obj bind obj failed\n");
        return -1;
    }
    obj->bind_obj = (void*)bind;
    return 0;
}
