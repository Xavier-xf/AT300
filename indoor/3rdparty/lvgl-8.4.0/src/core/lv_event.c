...
static void (*lv_event_pressed_func)(lv_event_t* e) = NULL;
void lv_event_pressed_callback_register(void (*callback)(lv_event_t* e)) {
    lv_event_pressed_func = callback;
}
...
static lv_res_t event_send_core(lv_event_t * e)
{
    ...
    while(event_dsc && res == LV_RES_OK) {
        ...
        if ((lv_event_pressed_func) && ((event_dsc->filter & LV_EVENT_PREPROCESS) == 0) && (e->code == LV_EVENT_PRESSED)) {
            lv_event_pressed_func(e);
        }
