...
{
    static TKL_QUEUE_T *tkl_queue = NULL;
...
TUYA_WEAK_ATTRIBUTE OPERATE_RET tkl_queue_post(CONST TKL_QUEUE_HANDLE queue, VOID_T *data, UINT_T timeout)
{
    ...
    VOID_T *buf = (VOID_T *)malloc(tkl_queue->msgsize);
    if (NULL == buf) {
        return OPRT_MALLOC_FAILED;
    }

    memcpy(buf, (VOID_T *)data, tkl_queue->msgsize);
...
TUYA_WEAK_ATTRIBUTE OPERATE_RET tkl_queue_fetch(CONST TKL_QUEUE_HANDLE queue, VOID_T *msg, UINT_T timeout)
{
    ...
    if (!rpa_queue_timedpop(tkl_queue->queue, (VOID_T **)&buf, wait_ms)) {
        return OPRT_OS_ADAPTER_QUEUE_RECV_FAIL;
    }

    if (buf) {
        memcpy((VOID_T *)msg, buf, tkl_queue->msgsize);
        free(buf);
    }
