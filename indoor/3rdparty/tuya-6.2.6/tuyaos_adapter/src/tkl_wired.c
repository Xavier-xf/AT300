...
{
    extern CHAR_T s_net_device[32];
    char *dev = s_net_device;
    if (dev[0] && strlen(dev) < size)
    {
        strcpy(devname, dev);
        return 0;
    }
...
OPERATE_RET tkl_wired_get_ip(NW_IP_S *ip)
{
    ...
    if (TY_AF_INET == ip->type){
        ...
        memset( &ip->addr, 0x0, sizeof( ip->addr ) );
        memset(&ifr, 0x0, sizeof(ifr));
        strncpy(ifr.ifr_name, devname, sizeof(ifr.ifr_name) - 1);
...
{
    printf("wired not support get ipv6\r\n");
