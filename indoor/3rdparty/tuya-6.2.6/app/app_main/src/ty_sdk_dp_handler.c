...
STATIC VOID __tuya_json_write(CHAR_T *buffer)
{
    ...
    snprintf(config_path, 256, "%s/tuya_dp_cfg.json", s_ipc_storage);
...
int tuya_absent_mode_report(bool state)
{
    ...
    BOOL_T param = state;
	return tuya_ipc_dp_report(NULL, TUYA_DP_ABSENT_MODE, PROP_BOOL, &param, 1);
}
#endif

#ifdef TUYA_DP_DEVICE_ACTIVE
int tuya_device_active_report(void)
{
    ...
    BOOL_T param = 1;
	return tuya_ipc_dp_report(NULL, TUYA_DP_DEVICE_ACTIVE, PROP_BOOL, &param, 1);
}
#endif

#ifdef TUYA_DP_ABNORMAL_UNLOCK
int tuya_abnormal_unlock_report(void)
{
    ...
    BOOL_T param = 1;
	return tuya_ipc_dp_report(NULL, TUYA_DP_ABNORMAL_UNLOCK, PROP_BOOL, &param, 1);
}
#endif

#ifdef TUYA_DP_ALARM_DETAILED
int tuya_alarm_detailed_report(unsigned int device, unsigned int sensor)
{
    ...
    char str[8] = {0};
    sprintf(str, "%d", device * 2 + sensor);
    return tuya_ipc_dp_report(NULL, TUYA_DP_ALARM_DETAILED, PROP_ENUM, (void *)str, 1);
}
#endif
...
static int door_lock_dp[] = {
    ...
    TUYA_DP_DOOR_LOCK2,
#endif
#ifdef TUYA_DP_DOOR_LOCK3
    TUYA_DP_DOOR_LOCK3,
#endif
};
int tuya_door_lock_report(int index, bool state)
{
    if (tuya_online_status_get() == false && index < (sizeof(door_lock_dp) / sizeof(door_lock_dp[0])))
	{
		return -1;
	}
    BOOL_T param = state;
	return tuya_ipc_dp_report(NULL, door_lock_dp[index], PROP_BOOL, &param, 1);
}

#ifdef TUYA_DP_LOCK_SUPPORT
int tuya_lock_support_report(void)
{
    ...
    for (int i = 0; i < (sizeof(door_lock_dp) / sizeof(door_lock_dp[0])); i++)
    {
        char temp[8] = {0};
        sprintf(temp, "%d,", door_lock_dp[i]);
        strcat(dp_str, temp);
    }
    dp_str[strlen(dp_str) - 1] = '\0';
    // db_log_debug("dp_str:%s\n", dp_str);
	return tuya_ipc_dp_report(NULL, TUYA_DP_LOCK_SUPPORT, PROP_STR, dp_str, 1);
}
#endif // TUYA_DP_LOCK_SUPPORT

#endif // TUYA_DP_DOOR_LOCK

#ifdef TUYA_DP_SWITCH_CHANNEL
// 上传通道格式："{\"cmd\":1,\"cc\":1,\"chs\":[{\"id\":1,\"n\":\"door1\"},{\"id\":2,\"n\":\"door2\"}]}"
#define TUYA_CHANNEL_CMD_START "{\"cmd\":1,\"cc\":%d,\"chs\":["
#define TUYA_CHANNEL_CMD_VALID "{\"id\":%d,\"n\":\"%s\"},"
#define TUYA_CHANNEL_CMD_END "]}"

int tuya_channel_valid_report(int curr_ch, tuya_ch_info_t *info, int total)
{
    if (tuya_online_status_get() == false)
    {
        return -1;
    ...
    char dp_str[512] = {0};
    char temp_str[128] = {0};
    sprintf(dp_str, TUYA_CHANNEL_CMD_START, curr_ch);
    for (int i = 0; i < total; i++)
    {
        sprintf(temp_str, TUYA_CHANNEL_CMD_VALID, info[i].ch, info[i].name);
        strcat(dp_str, temp_str);
    }
    dp_str[strlen(dp_str) - 1] = '\0';
    strcat(dp_str, TUYA_CHANNEL_CMD_END); // strlen(dp_str) <= 255

    return tuya_ipc_dp_report(NULL, TUYA_DP_SWITCH_CHANNEL, PROP_STR, dp_str, 1);
}
#endif // TUYA_DP_SWITCH_CHANNEL