...
{
    respone_dp_value(TUYA_DP_SD_STATUS_ONLY_GET, 1);
}
#endif
...
#else
    tuya_event_cmd_send(TUYA_EVENT_CMD_DOOR_LOCK, (0 << 8) | (state ? 0x01 : 0x00));
#endif
}
#endif

#ifdef TUYA_DP_SWITCH_CHANNEL
STATIC VOID handle_DP_SWITCH_CHANNEL(IN TY_OBJ_DP_S *p_obj_dp)
{
#if 0
    respone_dp_str(p_obj_dp->dpid, p_obj_dp->value.dp_str);
#else
    const char *value = p_obj_dp->value.dp_str;
    const char *head_str = "{\"cmd\":1,\"cc\":";
    char buf[16] = {0};
    int channel = 0;
    if (strstr(value, head_str) == NULL)
    {
        db_log_error("String error ");
        return;
    }
    const char *start = value + strlen(head_str);
    const char *end = strchr(start, ',');
    strncpy(buf, start, end - start);
    channel = atoi(buf);
    tuya_event_cmd_send(TUYA_EVENT_CMD_SWITCH_CHANNEL, channel);
#endif
}
#endif

#ifdef TUYA_DP_ABSENT_MODE
STATIC VOID handle_DP_ABSENT_MODE(IN TY_OBJ_DP_S *p_obj_dp)
{
    BOOL_T state = check_dp_bool_invalid(p_obj_dp);
    tuya_event_cmd_send(TUYA_EVENT_CMD_ABSENT_MODE, state);
}
#endif

#ifdef TUYA_DP_DOOR_LOCK1
STATIC VOID handle_DP_DOOR_LOCK1(IN TY_OBJ_DP_S *p_obj_dp)
{
    BOOL_T state = check_dp_bool_invalid(p_obj_dp);
#if 0
    respone_dp_bool(p_obj_dp->dpid, state);
#else
    tuya_event_cmd_send(TUYA_EVENT_CMD_DOOR_LOCK, (1 << 8) | (state ? 0x01 : 0x00));
#endif
}
#endif
...
STATIC VOID handle_DP_DOOR_LOCK2(IN TY_OBJ_DP_S *p_obj_dp)
{
    ...
    respone_dp_bool(p_obj_dp->dpid, state);
#else
    tuya_event_cmd_send(TUYA_EVENT_CMD_DOOR_LOCK, (2 << 8) | (state ? 0x01 : 0x00));
#endif
}
#endif

#ifdef TUYA_DP_DOOR_LOCK3
STATIC VOID handle_DP_DOOR_LOCK3(IN TY_OBJ_DP_S *p_obj_dp)
{
    BOOL_T state = check_dp_bool_invalid(p_obj_dp);
#if 0
    respone_dp_bool(p_obj_dp->dpid, state);
#else
    tuya_event_cmd_send(TUYA_EVENT_CMD_DOOR_LOCK, (3 << 8) | (state ? 0x01 : 0x00));
#endif
}
#endif
...
#ifdef TUYA_DP_DOOR_LOCK
    {TUYA_DP_DOOR_LOCK,             handle_DP_DOOR_LOCK},
#endif

#ifdef TUYA_DP_SWITCH_CHANNEL
    {TUYA_DP_SWITCH_CHANNEL,        handle_DP_SWITCH_CHANNEL},
#endif

#ifdef TUYA_DP_ABSENT_MODE
    {TUYA_DP_ABSENT_MODE,           handle_DP_ABSENT_MODE},
#endif

#ifdef TUYA_DP_DOOR_LOCK1
    {TUYA_DP_DOOR_LOCK1,            handle_DP_DOOR_LOCK1},
#endif

#ifdef TUYA_DP_DOOR_LOCK2
    {TUYA_DP_DOOR_LOCK2,            handle_DP_DOOR_LOCK2},
#endif

#ifdef TUYA_DP_DOOR_LOCK3
    {TUYA_DP_DOOR_LOCK3,            handle_DP_DOOR_LOCK3},
#endif
...
VOID TUYA_IPC_handle_dp_cmd_objs(IN CONST TY_RECV_OBJ_DP_S *dp_rev)
{
    ...
    db_log_debug("TUYA: dpid:%d type:%d time:%d", dp_data->dpid, dp_data->type, dp_data->time_stamp);
    if (dp_data->type == PROP_BOOL) {
        db_log_debug("value:%d", dp_data->value.dp_bool);
    } else if (dp_data->type == PROP_VALUE) {
        db_log_debug("value:%d", dp_data->value.dp_value);
    } else if (dp_data->type == PROP_STR) {
        db_log_debug("value:%s", dp_data->value.dp_str);
    } else if (dp_data->type == PROP_ENUM) {
        db_log_debug("value:%d", dp_data->value.dp_enum);
    }
