...
#define AUTH_START()                     \
    onvif_data->auth = ONVIF_AUTH_TOKEN; \
    auth:

#define AUTH_TOKEN() \
    if (onvif_data->auth == ONVIF_AUTH_TOKEN)

#define AUTH_SWITCH(service)                                                                                       \
    if (result)                                                                                                    \
    {                                                                                                              \
        switch (onvif_data->auth)                                                                                  \
        {                                                                                                          \
        case ONVIF_AUTH_TOKEN:                                                                                     \
            if (strstr(onvif_data->httphead, "Unauthorized"))                                                      \
            {                                                                                                      \
                if (getHttpHeadParserDigest(onvif_data, service) == 0)                                             \
                    onvif_data->auth = ONVIF_AUTH_DIGEST;                                                          \
                else                                                                                               \
                    onvif_data->auth = ONVIF_AUTH_NONE;                                                            \
                goto auth;                                                                                         \
            }                                                                                                      \
            break;                                                                                                 \
        case ONVIF_AUTH_NONE:                                                                                      \
            if (strstr(onvif_data->httphead, "Unauthorized") && getHttpHeadParserDigest(onvif_data, service) == 0) \
            {                                                                                                      \
                onvif_data->auth = ONVIF_AUTH_DIGEST;                                                              \
                goto auth;                                                                                         \
            }                                                                                                      \
            break;                                                                                                 \
        case ONVIF_AUTH_DIGEST:                                                                                    \
            break;                                                                                                 \
        default:                                                                                                   \
            break;                                                                                                 \
        }                                                                                                          \
...
xmlDocPtr sendCommandToCamera(struct OnvifData *onvif_data, char *cmd, char *xaddrs);
...
void addHttpHeader(struct OnvifData *onvif_data, xmlDocPtr doc, xmlNodePtr root, char *xaddrs, char *post_type, char cmd[], int cmd_length);
...
// 将字符串转成md5检验码
static bool str_to_md5(const char *str, char *md5_buf, int buf_size)
{
    char buf[1024] = {0};
    sprintf(buf, "echo -n \"%s\" | md5sum | awk '{print $1}'", str);
    FILE *fp = popen(buf, "r");
    if (fp == NULL)
    {
        return false;
...
// 解析http消息
static bool http_msg_parsing(const char *buf, const char *type, char *data, int size)
{
    char *start;
    char *end;
    start = strstr(buf, type); // 找到消息类型标识
    if (start == NULL)
    {
        return false;
    }
...
static int getHttpHeadParserDigest(struct OnvifData *onvif_data, const char *service)
{
    char HA1[36] = {0};
    char HA2[36] = {0};
    char response[36] = {0};
    ...
    if (end == NULL || (end - start) >= sizeof(method))
    {
        return -1;
    }
    ...
    if (end == NULL || (end - start) >= sizeof(uri))
    {
        return -1;
    }
    memcpy(uri, start, end - start);
    // WWW-Authenticate
    start = strstr(onvif_data->httphead, "WWW-Authenticate: ");
    if (start == NULL)
    {
        return -1;
    }
    start += 18;
    // qop
    http_msg_parsing(start, "qop", qop, sizeof(qop));
    // realm
    http_msg_parsing(start, "realm", realm, sizeof(realm));
    // nonce
    http_msg_parsing(start, "nonce", nonce, sizeof(nonce));
    // opaque
    http_msg_parsing(start, "opaque", opaque, sizeof(opaque));
    // cnonce
    for (int i = 0; i < sizeof(nonce) / 2; i++)
    {
        cnonce[i * 2 + 0] = nonce[i * 2 + 1];
        cnonce[i * 2 + 1] = nonce[i * 2 + 0];
    }
    // HA1 = MD5(<username>:<realm>:<password>)
    sprintf(buffer, "%s:%s:%s", onvif_data->username, realm, onvif_data->password);
    str_to_md5(buffer, HA1, sizeof(HA1));
    // HA2 = MD5(<method>:<uri>)
    sprintf(buffer, "%s:%s", method, uri);
    str_to_md5(buffer, HA2, sizeof(HA2));
    // Response = MD5(HA1:<nonce>:<nc>:<cnonce>:<qop>:HA2)
    sprintf(buffer, "%s:%s:%s:%s:%s:%s", HA1, nonce, nc, cnonce, qop, HA2);
    str_to_md5(buffer, response, sizeof(response));
    if (opaque[0] != 0)
    {
        sprintf(onvif_data->digest, "\r\nAuthorization: Digest username=\"%s\",realm=\"%s\",qop=\"%s\",algorithm=MD5,uri=\"%s\",nonce=\"%s\",nc=00000001,cnonce=\"%s\",opaque=\"%s\",response=\"%s\"",
                onvif_data->username, realm, qop, uri, nonce, cnonce, opaque, response);
    }
    else
    {
        sprintf(onvif_data->digest, "\r\nAuthorization: Digest username=\"%s\",realm=\"%s\",qop=\"%s\",algorithm=MD5,uri=\"%s\",nonce=\"%s\",nc=00000001,cnonce=\"%s\",response=\"%s\"",
                onvif_data->username, realm, qop, uri, nonce, cnonce, response);
    }
    return 0;
}
...
int getNetworkInterfaces(struct OnvifData *onvif_data)
{
    ...
    char cmd[4096] = {0};
    addHttpHeader(onvif_data, doc, root, onvif_data->xaddrs, onvif_data->device_service, cmd, 4096);
    xmlDocPtr reply = sendCommandToCamera(onvif_data, cmd, onvif_data->xaddrs);
    if (reply != NULL)
    {
    ...
    AUTH_SWITCH(onvif_data->device_service);
    return result;
}
...
int getVideoEncoderConfigurationOptions(struct OnvifData *onvif_data)
{
    ...
    char cmd[4096] = {0};
    addHttpHeader(onvif_data, doc, root, onvif_data->xaddrs, onvif_data->media_service, cmd, 4096);
    xmlDocPtr reply = sendCommandToCamera(onvif_data, cmd, onvif_data->xaddrs);
    if (reply != NULL)
    {
        xmlChar *width = NULL;
    ...
    AUTH_SWITCH(onvif_data->media_service);
    return result;
}
...
int getOptions(struct OnvifData *onvif_data)
{
    ...
    char cmd[4096] = {0};
    addHttpHeader(onvif_data, doc, root, onvif_data->xaddrs, onvif_data->imaging_service, cmd, 4096);
    xmlDocPtr reply = sendCommandToCamera(onvif_data, cmd, onvif_data->xaddrs);
    if (reply != NULL)
    {
        xmlChar *xpath;
    ...
    AUTH_SWITCH(onvif_data->imaging_service);
    return result;
}
...
int getStatus(struct OnvifData *onvif_data)
{
    ...
    char cmd[4096] = {0};
    addHttpHeader(onvif_data, doc, root, onvif_data->xaddrs, onvif_data->ptz_service, cmd, sizeof(cmd));
    xmlDocPtr reply = sendCommandToCamera(onvif_data, cmd, onvif_data->xaddrs);
    if (reply != NULL)
    {
    ...
    AUTH_SWITCH(onvif_data->ptz_service);
    return result;
}
...
int absoluteMove(float x_position, float y_position, struct OnvifData *onvif_data)
{
    ...
    char cmd[4096] = {0};
    addHttpHeader(onvif_data, doc, root, onvif_data->xaddrs, onvif_data->ptz_service, cmd, 4096);
    xmlDocPtr reply = sendCommandToCamera(onvif_data, cmd, onvif_data->xaddrs);
    if (reply != NULL)
    {
        result = checkForXmlErrorMsg(reply, onvif_data->last_error);
...
xmlDocPtr sendCommandToCamera(struct OnvifData *onvif_data, char *cmd, char *xaddrs)
{
    ...
    if (xml_start > 1024)
        return NULL;
    char *http_header = onvif_data->httphead; // char http_header[1024];
...
int checkForXmlErrorMsg(xmlDocPtr doc, char error_msg[1024])
{
    ...
    else
    {
        ...
        if (root)
        {
            ...
            for (xmlNodePtr msg = root->xmlChildrenNode; msg; msg = msg->xmlChildrenNode)
            {
                if ((!xmlStrcmp(msg->name, (const xmlChar *)"error")) ||
                    (xmlStrstr(msg->content, (const xmlChar *)"Unauthorized")))
                {
                    memset(error_msg, 0, sizeof(error_msg));
                    xmlChar *content = xmlNodeGetContent(msg);
                    if (!content)
                    {
                        return -1;
                    }
                    strcpy(error_msg, (char *)content);
                    xmlFree(content);
                    return -1;
                }
            }
...
void addHttpHeader(struct OnvifData *onvif_data, xmlDocPtr doc, xmlNodePtr root, char *xaddrs, char *post_type, char cmd[], int cmd_length)
{
    ...
    const char *digest = onvif_data->digest;
    int p = strlen(post_type) + 1;
    int h = strlen(host) + 1;
    int c = sizeof(content);
    int cl = sizeof(content_length);
    int dl = onvif_data->auth == ONVIF_AUTH_DIGEST ? strlen(digest) + 1 : 0;
    int cmd_size = p + c + h + cl + dl + xml_size_length + size + 1;
    int i;
    int s;
    for (i = 0; i < p - 1; i++)
        cmd[i] = post_type[i];
...
int broadcast(struct OnvifSession *onvif_session)
{
    ...
    setSocketOptions(broadcast_socket, onvif_session->preferred_network_address);
    for (int k = 0; k < ONVIF_MAX_DEVICES; k++)
    {
        for (int j = 0; j < ONVIF_BUF_SIZE; j++)
        {
...
int setSocketOptions(int socket, char *preferred_network_address)
{
    struct timeval tv;
    tv.tv_sec = 0;
    tv.tv_usec = 500000;
...
void initializeSession(struct OnvifSession *onvif_session)
{
    ...
    xmlInitParser();
    for (int i = 0; i < ONVIF_MAX_INTERFACES; i++)
    {
