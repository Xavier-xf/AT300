...
#define ONVIF_MAX_DEVICES 128   // 设备发现数量
#define ONVIF_BUF_SIZE 8192     // ONVIF响应数据
#define ONVIF_MAX_INTERFACES 16 // 本地网卡接口数量
    ...
    struct OnvifSession
    {
        char buf[ONVIF_MAX_DEVICES][ONVIF_BUF_SIZE];
        int len[ONVIF_MAX_DEVICES];
        char uuid[47];
        int discovery_msg_id;
        char preferred_network_address[16];
        char active_network_interfaces[ONVIF_MAX_INTERFACES][1024];
...
#define getFirstProfileToken(a) getProfileToken(a, 0)
    ...
    LIBRARY_API int setSocketOptions(int socket, char *preferred_network_address);
